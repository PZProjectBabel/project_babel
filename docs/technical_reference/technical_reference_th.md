...
- [4. ข้อกำหนดข้อมูล](#4-ข้อกำหนดข้อมูล)
  ...
  - [4.4 เครื่องสถานะ](#44-เครื่องสถานะ)
    ...
    - [TranslationData สถานะการตรวจสอบการแปล](#translationdata-สถานะการตรวจสอบการแปล)
    - [ModInfo.needsUpdate การตัดสินใจอัปเดต](#modinfoneedsupdate-การตัดสินใจอัปเดต)
- [5. คำอธิบายการตั้งค่า](#5-คำอธิบายการตั้งค่า)
  - [5.1 `config/config.json` — การตั้งค่าหลักของไปป์ไลน์](#51-configconfigjson-การตั้งค่าหลักของไปป์ไลน์)
    - [5.1.1 `LLM` — การตั้งค่าโมเดลภาษาขนาดใหญ่](#511-llm-การตั้งค่าโมเดลภาษาขนาดใหญ่)
...
**ฟังก์ชัน**: ค้นหาข้อมูลเมตาดีเทลของโมดจำนวนมากผ่าน Steam Web API เพื่อตรวจสอบว่าโมดใดต้องการอัปเดต

หลังจากได้รับรายการ Mod ID แล้ว ไปป์ไลน์จำเป็นต้องทราบข้อมูลพื้นฐานของแต่ละโมด ได้แก่ ชื่อ ผู้สร้าง เวลาอัปเดตล่าสุด ฯลฯ ข้อมูลเหล่านี้ได้มาจากอินเทอร์เฟซ `ISteamRemoteStorage/GetPublishedFileDetails/v1/` ของ Steam อย่างเป็นทางการ

**รายละเอียดการทำงาน**:
- **คำขอแบบแบ่งกลุ่ม**: Steam API มีข้อจำกัดจำนวนการเรียกใช้ต่อครั้ง ดังนั้นไปป์ไลน์จึงส่งคำขอเป็นชุดตาม `steamApiChunkSize` (ค่าเริ่มต้น 100) โดยเว้นระยะห่างระหว่างชุดอย่างเหมาะสมเพื่อหลีกเลี่ยงการถูกจำกัดอัตรา
- **กลไกการทนต่อข้อผิดพลาด**: หากชุดคำขอ 5 ชุดติดต่อกันล้มเหลวทั้งหมด (อาจเป็นปัญหาเครือข่ายหรือ API ไม่พร้อมใช้งานชั่วคราว) ไปป์ไลน์จะยุติการค้นหาและเก็บข้อมูลส่วนที่ดึงมาได้สำเร็จไว้ แทนที่จะทิ้งผลลัพธ์ทั้งหมด
- **การแมปฟิลด์สำคัญ**:
- `consumer_app_id`: ตรวจสอบว่าไอเท็มนี้เป็นของ Project Zomboid หรือไม่ (App ID = `108600`) โมดที่ไม่ใช่ PZ จะถูกทำเครื่องหมายเป็น `isAvailable = false` และข้ามการดาวน์โหลด
- `time_updated`: เวลาอัปเดตล่าสุดที่บันทึกโดย Steam เปรียบเทียบกับ `timeModUpdated` ในแคช หากค่าแรกล่าสุดกว่า ให้ทำเครื่องหมาย `needsUpdate = true` แสดงว่าเนื้อหาโมดอาจเปลี่ยนแปลง ต้องแยกและแปลใหม่
- `title` → แมปเป็น `modName` (ชื่อโมด)
- `creator` → รับชื่อเล่นผู้สร้างผ่านอินเทอร์เฟซผู้ใช้ Steam

### 3.5 SteamCmdBootstrapper (`SteamCmdBootstrapperService`)

**ฟังก์ชัน**: เตรียมรันไทม์ steamcmd ที่ทำงานบนแพลตฟอร์มปัจจุบันก่อนเริ่มดำเนินการดาวน์โหลดทั้งหมด

- **Linux**: ล้างไฟล์รันไทม์เก่าใน `src/3rd_party/steamcmd/` ดาวน์โหลดและแตกไฟล์ `steamcmd_linux.tar.gz` อย่างเป็นทางการ และตั้งค่าสิทธิ์การดำเนินการให้กับ `steamcmd.sh`
- **Windows**: ไม่ดาวน์โหลดแพ็กเกจบีบอัด ดำเนินการ `steamcmd.exe +quit` ที่มาพร้อมกับที่เก็บใน `src/3rd_party/steamcmd/` โดยตรง เพื่อให้ SteamCMD อัปเดตตัวเอง
- **การจัดการข้อผิดพลาด**: การดาวน์โหลด การแตกไฟล์ หรือการตรวจสอบไฟล์ปฏิบัติการล้มเหลวจะยุติไปป์ไลน์ เพื่อป้องกันการใช้รันไทม์ที่ไม่สมบูรณ์ในขั้นตอนดาวน์โหลด

### 3.5.1 ModDownloader (`ModDownloaderService`)

**ฟังก์ชัน**: ดาวน์โหลดไฟล์โมดจาก Steam Workshop โดยใช้เครื่องมือบรรทัดคำสั่ง steamcmd

[steamcmd](https://developer.valvesoftware.com/wiki/SteamCMD) เป็นไคลเอนต์ Steam แบบบรรทัดคำสั่งที่ Valve ให้ไว้อย่างเป็นทางการ รองรับการเข้าสู่ระบบแบบไม่ระบุตัวตนและดาวน์โหลดเนื้อหา Workshop ไปป์ไลน์เรียกใช้ steamcmd เพื่อดำเนินการดาวน์โหลดไฟล์โมดเป็นชุด

**ขั้นตอนการดาวน์โหลด**:
1. **คัดลอก steamcmd**: คัดลอก `src/3rd_party/steamcmd/` ไปยังไดเรกทอรีชั่วคราวเฉพาะของชุดงาน เนื่องจากแต่ละชุดดาวน์โหลดจะเริ่มกระบวนการ steamcmd แยกกัน หากหลายกระบวนการแชร์ไฟล์เดียวกันอาจเกิดความขัดแย้ง
2. **ดำเนินการคำสั่งดาวน์โหลด**: รัน `steamcmd +login anonymous +workshop_download_item 108600 <modId> +quit` โดยที่ `108600` คือ App ID ของ Project Zomboid และ `anonymous` หมายถึงเข้าสู่ระบบแบบไม่ระบุตัวตน (การดาวน์โหลด Workshop ไม่จำเป็นต้องใช้บัญชี)
3. **ตรวจสอบผลลัพธ์**: วิเคราะห์เอาต์พุตมาตรฐานและบันทึกของ steamcmd เพื่อระบุไดเรกทอรีเอาต์พุตจริงของ Workshop ก่อนย้ายผลลัพธ์ที่ดาวน์โหลด หากล้มเหลว ให้ลองอีกครั้งตามกลยุทธ์การดาวน์โหลดซ้ำของ Steam
4. **การดาวน์โหลดต่อเนื่อง**: โมดที่ดาวน์โหลดสำเร็จแล้วจะถูกข้ามโดยอัตโนมัติ ไม่ดาวน์โหลดซ้ำ

**แหล่งที่มาของรันไทม์**: แต่ละชุดดาวน์โหลดจะคัดลอกรันไทม์ที่เตรียมโดย `SteamCmdBootstrapper` จาก `src/3rd_party/steamcmd/` เพื่อหลีกเลี่ยงการใช้ไดเรกทอรีทำงานเดียวกันในชุดแบบขนาน

### 3.6 ContentExtractor (`ContentExtractorService`)

**ฟังก์ชัน**: แยกและดึงเนื้อหาข้อความที่แปลได้ทั้งหมดจากไฟล์โมดที่ดาวน์โหลด เป็นขั้นตอนสำคัญของไปป์ไลน์ในการ "ทำความเข้าใจโมด"

โมดของ Project Zomboid จัดเก็บข้อความแปลในไดเรกทอรีเฉพาะ ภารกิจของ `ContentExtractor` คือการสำรวจไดเรกทอรีเหล่านี้ แยกวิเคราะห์ไฟล์สองรูปแบบคือ TXT (รูปแบบ Lua) และ JSON และดึงคู่คีย์-ค่า "ต้นฉบับ → คำแปล" แต่ละรายการ
...
- **การตรวจสอบด้วย LLM**: เรียกใช้โมเดล `deepseek-flash` โดยใช้ JSON Mode เพื่อส่งออกผลการตรวจสอบที่มีโครงสร้าง (รวมถึงผลการตัดสินและระดับความเชื่อมั่น)
...
| model มี `flash` | 128 | 2000 | ความสามารถพร้อมกันสูงของ DeepSeek V4 Flash |
...
**เจตนาการออกแบบ — รูปแบบแคชภายใน**: การเลือกรูปแบบ `key::lang::status = "value"` แทน JSON เป็นรูปแบบแคชภายใน เนื่องจากรูปแบบนี้มีความหนาแน่นของข้อมูลสูง ทำให้สามารถแสดงข้อมูลบริบทเพิ่มเติมบนหน้าจอเมื่อผู้ใช้ตรวจสอบเนื้อหาแปลด้วยตนเอง
...
โมดูลต่อไปนี้ทำงานแยกจากไปป์ไลน์การแปล ไม่ได้อยู่ใน `TranslationPipeline.slnx` แต่ละโมดูลจะถูกเรียกใช้ผ่าน `dotnet run --project` หรือ GitHub Actions

### WorkshopMonitor (`WorkshopMonitorService`)

**หน้าที่**: ตรวจสอบม็อดใหม่ที่อัปโหลดบน Steam Workshop ตามกำหนดเวลา กรองม็อดที่มีจำนวนผู้ติดตามสูงโดยอัตโนมัติ และเพิ่มลงในรายการคำขอแปล

**วิธีการทำงาน**: ถูกเรียกใช้ตามกำหนดเวลาผ่าน GitHub Actions `.github/workflows/monitor-workshop.yml` (ทุกวันเวลา 00:00 น. ตามเวลาปักกิ่ง) หรือเรียกใช้ในเครื่องด้วย `dotnet run --project src/WorkshopMonitor/WorkshopMonitor.csproj`

**ขั้นตอนการทำงาน**:
1. **ดึงรายการ**: ดึง ID ม็อดที่มีแท็ก Build 42 (ยกเว้นแท็ก Language/Translation) จากหน้า "ล่าสุด" ของ Steam Workshop แบบแบ่งหน้า
2. **วิเคราะห์เวลา**: สืบค้นเวลาที่เผยแพร่ของแต่ละม็อดเป็นชุดผ่าน Steam Web API เปรียบเทียบกับเวลาที่รันครั้งล่าสุดในแคช เพื่อระบุม็อดใหม่
3. **กรองตามจำนวนสมาชิก**: เรียก Steam API อีกครั้งเพื่อสอบถามจำนวนสมาชิกของม็อดทั้งหมดที่แคชไว้ กรองเฉพาะม็อดที่เกินเกณฑ์ (500)
4. **รวมและส่งออก**: รวม ID ม็อดที่กรองแล้ว (ลบรายการซ้ำ) ไปยัง `config/request_for_translation.txt` เพื่อให้ `ModIdCollector` ในไปป์ไลน์นำไปใช้

**พารามิเตอร์แบบฮาร์ดโค้ด**: AppId=108600, MinSubs=500, SafetyPages=5 (จำนวนหน้าที่ดึงเพิ่มเติมหลังจากถึง Timestamp ล่าสุด), PageSize=30, Lookback=48h

**รูปแบบแคช**: `data/monitor_cache.bin` — ไฟล์ไบนารีที่บีบอัดด้วย Zstd, ลำดับ int64 แบบ little-endian: `[lastRunUnixSec][modId0][timeCreated0][modId1][timeCreated1]...` ใช้แผนการบีบอัด `ZstdSharp` ร่วมกับ `BinaryEmbeddingSerializer`

**การอ่านคีย์**: Steam API Key อ่านจากฟิลด์ `STEAM_KEY` ใน `config/secrets.json` หรือจากตัวแปรสภาพแวดล้อม `STEAM_KEY` / `STEAM_API_KEY` (รูปแบบเดียวกับ `ConfigReader`)

### DocGenerator

**หน้าที่**: ตัวสร้างเอกสารหลายภาษาโดยใช้ LLM สร้าง README, คู่มือการมีส่วนร่วม และเอกสารอ้างอิงทางเทคนิคสำหรับแต่ละภาษาจากเทมเพลตภาษาจีน

**วิธีการทำงาน**: โปรเจกต์อิสระ `src/DocGenerator/DocGenerator.csproj` ดำเนินการโดย `dotnet run --project src/DocGenerator/DocGenerator.csproj`
...
└──(LLM ตรวจสอบ: ไม่แน่ใจ, ค่าความเชื่อมั่น<0.7)──→ NEEDVERIFICATION (รอการตรวจสอบโดยมนุษย์)

ACCEPTED ──(เกินระยะเวลาแคช 90 วัน)──→ NEEDVERIFICATION (ตรวจสอบซ้ำเป็นระยะ)
```

- **UNKNOWN**: ม็อดที่เพิ่งค้นพบ ยังไม่ผ่านการตรวจสอบเนื้อหา
- **NEEDVERIFICATION**: จำเป็นต้องตรวจสอบ (หรือตรวจสอบซ้ำ) ไปป์ไลน์จะเรียกใช้ LLM เพื่อสแกนความปลอดภัยของเนื้อหาของม็อดนั้น
- **ACCEPTED**: ผ่านการตรวจสอบ เนื้อหาของม็อดนี้ปลอดภัย สามารถแปลได้ตามปกติ
- **REJECTED**: ไม่ผ่านการตรวจสอบ ม็อดนี้มีเนื้อหาที่ละเมิด ข้ามการแปล

#### TranslationData สถานะการตรวจสอบการแปล

ความน่าเชื่อถือของข้อมูลการแปลแต่ละรายการจะถูกแยกแยะโดยใช้เครื่องหมาย `isVerified`:

| สถานะ | `isVerified` | ความหมาย |
|------|-------------|------|
| ตรวจสอบแล้ว (แปลโดยมนุษย์) | `true` | มาจากม็อดอ้างอิงการแปล แปลและยืนยันโดยมนุษย์ |
| ยังไม่ตรวจสอบ (แปลโดย AI) | `false` | แปลโดย LLM โดยอัตโนมัติ ทำเครื่องหมายเป็น `unverified` ยังไม่ผ่านการตรวจสอบโดยมนุษย์ |
| รอการแปล | ไม่มีข้อความ | ยังไม่ได้แปล ไม่มีข้อความที่สอดคล้องใน `translationValues` |

#### ModInfo.needsUpdate การตัดสินใจอัปเดต

ไม่ว่าม็อดจำเป็นต้องถูกสกัดและแปลใหม่หรือไม่ ให้ตัดสินตามกฎต่อไปนี้:
- Steam `time_updated` ช้ากว่า `timeModUpdated` ที่แคชไว้ → `needsUpdate = true` (ผู้เขียนม็อดได้เผยแพร่การอัปเดต)
- ม็อดที่เข้าถึงได้ซึ่งไม่มีรายการแปลในแคช → `needsUpdate = true` (กำลังประมวลผลม็อดนี้เป็นครั้งแรก)
- หลังจากสกัดม็อดแล้วมีรายการแปล 0 รายการ → ตั้งค่าสถานะการตรวจสอบเนื้อหาเป็น `ACCEPTED` โดยตรง (ม็อดนี้ไม่มีเนื้อหาที่แปลได้ ไม่จำเป็นต้องแปล)

---

## 5. คำอธิบายการตั้งค่า

ภายในไดเรกทอรี `config/` มีไฟล์การตั้งค่า 5 ไฟล์ แบ่งตามหน้าที่เป็น การควบคุมไปป์ไลน์ การจัดการคีย์ คำจำกัดความภาษา คลังข้อมูลอ้างอิง และคำขอแปล

### 5.1 `config/config.json` — การตั้งค่าหลักของไปป์ไลน์

ไฟล์ควบคุมหลักของไปป์ไลน์แปลทั้งหมด ทุกฟิลด์จำเป็นต้องกรอก ยกเว้นที่ระบุว่า "ไม่บังคับ"

#### 5.1.1 `LLM` — การตั้งค่าโมเดลภาษาขนาดใหญ่

| ฟิลด์ | ประเภท | ค่าเริ่มต้น | คำอธิบาย |
|------|------|--------|------|
| `api_endpoint` | string | `https://api.deepseek.com/chat/completions` | ที่อยู่ API ของ LLM รองรับโปรโตคอล OpenAI Chat Completions |
| `model` | string | `deepseek-flash` | ชื่อโมเดล ค่าที่มี `flash` จะเรียกใช้ profile concurrency อัตโนมัติที่สอดคล้องกัน |
