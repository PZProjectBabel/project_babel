...
- **Перевірка LLM**: Викликається модель `deepseek-flash`, використовується JSON Mode для виведення структурованого висновку перевірки (з результатом і рівнем впевненості).
...
| model містить `flash` | 128 | 2000 | Висока здатність паралельності DeepSeek V4 Flash |
...
| `model` | string | `deepseek-flash` | Назва моделі. Значення, що містять `flash`, активують відповідний автоматичний профіль паралельності |
