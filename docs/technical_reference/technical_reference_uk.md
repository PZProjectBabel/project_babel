...
**Функція**: Готує платформозалежне середовище виконання steamcmd перед початком будь-яких операцій завантаження.

- **Linux**: Очищає старі файли середовища виконання в `src/3rd_party/steamcmd/`, завантажує та розпаковує офіційний `steamcmd_linux.tar.gz`, встановлює права на виконання для `steamcmd.sh`.
- **Windows**: Без завантаження архіву; безпосередньо виконує наданий у репозиторії `steamcmd.exe +quit` у `src/3rd_party/steamcmd/`, дозволяючи SteamCMD самооновлюватися.
- **Обробка помилок**: Збій завантаження, розпакування або перевірки виконуваного файлу перериває конвеєр, щоб запобігти використанню неповного середовища виконання на етапі завантаження.
