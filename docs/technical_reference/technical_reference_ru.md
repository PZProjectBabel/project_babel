...
**Функция**: Подготавливает платформозависимую среду выполнения steamcmd перед началом любых операций загрузки.

- **Linux**: Очищает старые файлы среды выполнения в `src/3rd_party/steamcmd/`, загружает и распаковывает официальный `steamcmd_linux.tar.gz`, устанавливает права на выполнение для `steamcmd.sh`.
- **Windows**: Без загрузки архива; напрямую выполняет поставляемый с репозиторием `steamcmd.exe +quit` в `src/3rd_party/steamcmd/`, позволяя SteamCMD самообновляться.
- **Обработка ошибок**: Сбой загрузки, распаковки или проверки исполняемого файла прерывает конвейер, чтобы предотвратить использование неполной среды выполнения на этапе загрузки.
