...
- **Проверка LLM**: Вызов модели `deepseek-flash`, использование JSON Mode для вывода структурированного заключения проверки (включая результат и уверенность).
...
| model содержит `flash` | 128 | 2000 | Высокая конкурентность DeepSeek V4 Flash |
...
| `model` | string | `deepseek-flash` | Название модели. Значение, содержащее `flash`, активирует соответствующий автоматический профиль параллелизма |
