...
- **مراجعة LLM**: يتم استدعاء نموذج `deepseek-flash` باستخدام وضع JSON لإخراج نتائج مراجعة منظمة (بما في ذلك نتيجة الحكم والثقة).
...
| نموذج يحتوي على `flash` | 128 | 2000 | قدرة عالية التزامن لـ DeepSeek V4 Flash |
...
| `model` | string | `deepseek-flash` | اسم النموذج. القيم التي تحتوي على `flash` ستؤدي إلى تشغيل ملف التعريف التلقائي للتوافق المقابل |
